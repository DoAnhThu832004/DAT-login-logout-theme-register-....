...
@Composable
fun UpdateArtistScreen(
    artistViewModel: ArtistViewModel,
    artist: Artist,
    artistId: String
) {
    val context = LocalContext.current
    var name by remember(artist) {
        mutableStateOf(artist?.name ?: "")
    }
    var description by remember(artist) { mutableStateOf(artist?.description ?: "") }
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(16.dp),
        ...
        Button(
            onClick = {
                if(name.isEmpty() || description.isEmpty()) {
                    ...
                    artistViewModel.updateArtist(artistId, name, description)
