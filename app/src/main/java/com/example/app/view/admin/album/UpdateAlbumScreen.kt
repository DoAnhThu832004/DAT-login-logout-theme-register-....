...
@Composable
fun UpdateAlbumScreen(
    albumViewModel: AlbumViewModel,
    album: Album,
    albumId: String
) {
    val context = LocalContext.current
    var name by remember(album) {
        mutableStateOf(album?.name ?: "")
    }
    var description by remember(album) { mutableStateOf(album?.description ?: "") }
    var status by remember(album) { mutableStateOf(album?.status ?: "") }
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(16.dp),
        ...
        Button(
            onClick = {
                if(name.isEmpty() || description.isEmpty() || status.isEmpty()) {
                    ...
                    albumViewModel.updateAlbum(albumId, name, description, status)
